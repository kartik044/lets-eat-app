...
const Restaurantcard = (props ) => {
  const {name,cloudinaryImageId} = props.resdata.data
  return (
   <>
   <div className=' font-mono flex flex-col   w-52 h-80 bg-pink-50 shadow-md shadow-slate-400     translate-x-1 hover:shadow-xl hover:shadow-red-200 cursor-pointer  rounded-2xl    hover:duration-700 hover:-translate-y-2  ' >
     ...
     <div className='my-1 flex flex-col ' >
      ...
      <div className='mx-1 '>
      <div className='text-red-600 truncate text-lg '>{props?.resdata?.data?.cuisines.join(",")}</div>
      <div className='text-red-400    text-sm'>{props?.resdata?.data?.lastMileTravelString}</div>
      <div className='flex flex-row my-1 justify-between '>
      <span className='text-red-400 text-sm  '>{props?.resdata?.data?.avgRating} </span>
      <span className='text-red-400 text-sm  '>{props?.resdata?.data?.costForTwoString}  </span>
      </div>
